...
;;Signature: accuracy(guess, x)
;;Purpose: Get the absolut value of ((guess)^2 - x)
;;Type: [Number * Number -> Number]
;;Pre-condition: guess =/= 0
;;Tests: (accuracy 1 2) → 1
(define accuracy
  (lambda (guess x)
    (abs (- (square guess) x))))
...
(define sqrt2
  (lambda (x init epsilon)
    (car (find-first (sqrt-lzl x init) (lambda (y) (> epsilon (cdr y)))))
  )
)
...
(define get-value
  (lambda (assoc-list key)
    (cond ((empty? assoc-list) 'fail)
          ((not (symbol? (car (car assoc-list)))) 'fail)
...
(define collect-all-values-1
 (lambda (lists key)
   (if (empty? lists)
       '()
       (filter (lambda (x) (not (equal? x 'fail))) (cons (get-value (car lists) key) (collect-all-values-1 (cdr lists) key)))       
   )
 )
)
...
(define collect-all-values-2
 (lambda (lists key)
   (if (empty? lists)
       '()
       (filter (lambda (x) (not (equal? x '()))) (cons (get-value$ (car lists) key (lambda (x) x) (lambda () '())) (collect-all-values-2 (cdr lists) key)))
   )
 )
)
